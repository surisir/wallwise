...
export async function visualizeWallColor(file: File, color: { hex: string; name?: string; aiOnly?: boolean; areaIds?: string[]; targetPoints?: GuidancePoint[]; excludedPoints?: GuidancePoint[]; selectedMask?: string; excludedMask?: string; projectType?: "interior" | "exterior"; lightingValue?: number; lightingLabel?: string }): Promise<Blob> {
  ...
  if (color.projectType) form.append("project_type", color.projectType);
  ...
  if (color.excludedMask) form.append("excluded_mask", color.excludedMask);
